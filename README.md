...
# This repository contains MATLAB scripts and functions for processing magnetic data
collected by the Autonomous Underwater Vehicle (AUV) like *Sentry*.

# The tools provided here enable comprehensive analysis, including spin calibration, 
...
├── main_processing.m                    % Master driver to run the full magnetic data workflow
│
├── core/                                % Core data processing modules
│   ├── Mag_initialize_data.m            % Load and structure raw magnetometer & nav data
│   ├── Mag_spin_fitting.m               % Detect and fit spin calibration maneuvers
│   ├── Mag_fit_spin_comp.m              % Fit heading-dependent coefficients from spins
│   ├── Mag_process_segment.m            % Process a selected straight segment
│   ├── Mag_process_dive_with_spins.m    % Full dive processing including spin sections
│   ├── Mag_process_dive_without_spins.m % Full dive processing excluding spin sections
│   ├── Mag_temperature_corr.m           % Apply temperature corrections
│   ├── Mag_calib1520_sensors.m          % Sensor-specific calibration routine (CALIB1520)
│   ├── Mag_component_rotation.m         % Rotate XYZ to geographic NED frame
│   ├── Mag_plot_filter_components.m     % Plot filtered magnetic components
│   ├── Mag_find_spins.m                 % Manual spin detection
│   ├── Mag_find_straight_segment.m      % Identify straight-line segments
│   ├── Mag_select_dive_segment.m        % Determine usable dive segment 
│   ├── Mag_save_withspins.m             % Save dive data (with spins)
│   ├── Mag_save_nospins.m               % Save dive data (excluding spins)
│   ├── Mag_ask_yesno.m                  % Utility for yes/no prompts
│   ├── Mag_magfd.m                      % Calculate IGRF geomagnetic field vector
│   ├── Mag_position_plot.m              % Make better plots
│   ├── Mag_distance_along_track.m       % Compute along-track distance
│   ├── Mag_extract_sentry_id.m          % Extract vehicle metadata (e.g., dive ID)
│   ├── Mag_crossover_correction.m       % Perform crossover correction on anomalies
│   ├── Mag_guspi_upward.m               % 2-D Guspi (1987) upward continuation algorithm
│   ├── Mag_2d_upcont.m                  % Driver script for 2-D upward continuation & diagnostics
│
├── utils/                               % General utilities
│   ├── Mag_haversine.m                  % Compute great-circle distance between coords
│
└── README.md                            % Project documentation

#-------Pre-requisites
1) MATLAB (recommended version R2020b or newer)
...
7) Saving and I/O Utilities
    7.1) Mag_prompt_savefig.m
    - Asks the user whether to save a figure and performs the save to a user-defined output path.

    7.2) Mag_save_nospins.m
    - Saves processed dive data with spin maneuvers excluded.

    7.3) Mag_save_withspins.m
    ...
    7.5) Mag_extract_sentry_id.m
    - Extracts the Sentry dive ID from local file or filename structure.
