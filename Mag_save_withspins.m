function Mag_save_withspins(magData, sbeData, Fitting, file_prefix, output_file)
    % Processes and optionally saves dive data inclusing spin times
    ...
    %
    % Syntax:
    %   save_withspins(magData, sbeData, Fitting, file_prefix, output_file)
    %
    % Inputs:
...
if isfile(output_file)
    if Mag_ask_yesno(sprintf('File "%s" exists. Recompute? (y/n): ', output_file))
        fprintf('Recomputing with-spins output...\n');
        Mag_process_dive_with_spins(magData, sbeData, Fitting, file_prefix);
    else
        fprintf('Skipped recomputation of dive with spins.\n');
    end
else
    fprintf('\n')
    fprintf('Processing and saving dive including spins time...\n');
    Mag_process_dive_with_spins(magData, sbeData, Fitting, file_prefix);
end